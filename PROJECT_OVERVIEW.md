...
| **`dashbord-masi-hybrid-forecasting-01`** (this repo) | The product layer: a FastAPI backend + Next.js frontend that *reads* the engine's outputs, plus a RAG assistant that answers questions about the project. | FastAPI, Next.js/React/Tailwind, ChromaDB, sentence-transformers, optional Ollama LLM |

**Key relationship:** this dashboard does **not** re-implement any modelling. It
reads the canonical CSV/JSON artifacts produced by the engine's CLI. By default
it looks for the engine at `../masi-hybrid-forecasting` (next to this folder);
override with the `MASI_PROJECT_ROOT` environment variable in `.env`.

```
2-TimeSeriesProject/
├── masi-hybrid-forecasting/              ← engine
└── dashbord-masi-hybrid-forecasting-01/  ← this repo, reads ../masi-hybrid-forecasting
```
...
risk layer (VaR / ES / GARCH vol / risk regime)
      │
      ▼
canonical export  →  outputs/etape*/  +  reports/   (in the engine repo)
      │
      ▼   (THIS dashboard's backend reads these files)
...
learning is only evaluated *after* simple baselines. Performance is reported with
realistic transaction costs and a full metric set (directional accuracy, Sharpe,
Sortino, max drawdown, Calmar, Deflated Sharpe Ratio, regime-conditional
metrics). The headline result — **CNN-LSTM `base12` + HMM-gate**, DSR ≈ 0.997,
robust to 5/10/20 bps costs — is treated as **defensible under the study
protocol, not proof of live alpha**. Full rules live in the engine repo under
`docs/methodology.md` and `docs/anti_leakage.md`.

---

## 5. How to run the whole thing

**Step 1 — the engine** (produces the artifacts this dashboard needs):
...
- This dashboard, full setup → [`README.md`](README.md)
- Backend API & services → [`app/README.md`](app/README.md)
- RAG assistant (full engine) → [`rag_project/README.md`](rag_project/README.md)
- Frontend → [`frontend/README.md`](frontend/README.md)
- The forecasting engine → `../masi-hybrid-forecasting/PROJECT_OVERVIEW.md`
